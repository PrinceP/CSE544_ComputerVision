...
file = open('5/groundtruth_rect.txt', 'r')
# print(file.readline())
...
images = []
for filename in glob.iglob('5/**/*.jpg' , recursive = True):
...
p0 = [i for i in p0 if X1<=i[0,0]<=X2 and Y1<=i[0,1]<=Y2]
p0 =  np.array(p0)
...
for img in images:
    ...
    new_pt1 = M.dot(pt1)
    new_pt2 = M.dot(pt2)
    img = cv2.rectangle(img,(np.int32(new_pt1[0]),np.int32(new_pt1[1])),(np.int32(new_pt2[0]),np.int32(new_pt2[1])),(255,0,0),1)
